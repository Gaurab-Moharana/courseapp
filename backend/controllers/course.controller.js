...
export const updateCourse = async (req, res) => {
  ...
  try {
    ...
    if (!course) {
      return res
        .status(404)
        .json({ errors: "Can't update,course is created by other admin" });
    }
...
export const deleteCourse = async (req, res) => {
  ...
  try {
    ...
    if (!course) {
      return res
        .status(404)
        .json({ errors: "Can't delete, course is created by other admin" });
    }
    return res.status(200).json({ message: "Course deleted successfully" });
  } catch (error) {
    console.log("Error in deleting course", error);
    return res.status(500).json({ errors: "Error in course deleting" });
  }
};
...
export const courseDetails = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ errors: "Error in getting the course details" });
...
export const confirmPurchase = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: "Purchase confirmed", order });
  } catch (error) {
    console.log("Error confirming purchase", error);
    res.status(500).json({ errors: "Failed to confirm purchase" });
  }
};
